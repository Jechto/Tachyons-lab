...
export class DeckEvaluator {
  ...
  public getTrainingDistribution(): number[] {
    ...
    for (const card of this.deck) {
      ...
      if (idx !== undefined) {
        trainingDistribution[idx] += (card.cardBonus["Specialty Priority"] !== -1 ? card.cardBonus["Specialty Priority"] || 0 : 0) / 100;
      }
    }
  ...
  public evaluateStats(
    ...
    for (const card of this.deck) {
      ...
      const trainingDistributionForCard = idx !== -1 ? trainingDistribution[idx] : 0.2;

      const bondNeededToFriendship = Math.max(
        80 - (card.cardBonus["Initial Friendship Gauge"] !== -1 ? card.cardBonus["Initial Friendship Gauge"] || 0 : 0) - (card.eventsStatReward.Bond || 0),
        0
      );
      ...
      for (const t of ["Speed", "Stamina", "Power", "Guts", "Intelligence"]) {
        ...
        if (typeIdx !== undefined) {
          ...
          totalFacilityBonus[t]["Training Effectiveness"] +=
            (card.cardBonus["Training Effectiveness"] !== -1 ? card.cardBonus["Training Effectiveness"] || 0 : 0) * trainingDistribution[typeIdx];
        }
      }

      if (card.cardType.type === "Support" || card.cardType.type === "Unknown") {
        continue;
      }
      ...
      const bonusKeys = [
        ...
        "Friendship Bonus", "Mood Effect", "Wit Friendship Recovery", "Total support cards", "Turns To Max Bond"
      ];
      ...
      totalFacilityBonus[cardTypeKey]["Speed Bonus"] += 
        (card.cardBonus["Speed Bonus"] !== -1 ? card.cardBonus["Speed Bonus"] : 0);
      totalFacilityBonus[cardTypeKey]["Stamina Bonus"] += 
        (card.cardBonus["Stamina Bonus"] !== -1 ? card.cardBonus["Stamina Bonus"] : 0);
      totalFacilityBonus[cardTypeKey]["Power Bonus"] += 
        (card.cardBonus["Power Bonus"] !== -1 ? card.cardBonus["Power Bonus"] : 0);
      totalFacilityBonus[cardTypeKey]["Guts Bonus"] += 
        (card.cardBonus["Guts Bonus"] !== -1 ? card.cardBonus["Guts Bonus"] : 0);
      totalFacilityBonus[cardTypeKey]["Wit Bonus"] += 
        (card.cardBonus["Wit Bonus"] !== -1 ? card.cardBonus["Wit Bonus"] : 0);

      totalFacilityBonus[cardTypeKey]["Friendship Bonus"] += 
        (card.cardBonus["Friendship Bonus"] !== -1 ? card.cardBonus["Friendship Bonus"] || 0 : 0);
      totalFacilityBonus[cardTypeKey]["Mood Effect"] += 
        (card.cardBonus["Mood Effect"] !== -1 ? card.cardBonus["Mood Effect"] || 0 : 0);
      totalFacilityBonus[cardTypeKey]["Wit Friendship Recovery"] += 
        (card.cardBonus["Wit Friendship Recovery"] !== -1 ? card.cardBonus["Wit Friendship Recovery"] : 0);

      totalFacilityBonus[cardTypeKey]["Total support cards"] += 1;
      totalFacilityBonus[cardTypeKey]["Turns To Max Bond"] += card.turnsToMaxBond;

      eventEffectiveness += (card.cardBonus["Event Effectiveness"] !== -1 ? card.cardBonus["Event Effectiveness"] || 0 : 0) / 100;
      eventRecovery += (card.cardBonus["Event Recovery"] !== -1 ? card.cardBonus["Event Recovery"] || 0 : 0) / 100;
      raceBonus += (card.cardBonus["Race Bonus"] !== -1 ? card.cardBonus["Race Bonus"] || 0 : 0) / 100;
    }

    // Calculating time to max bond
    for (const cardType of ["Speed", "Stamina", "Power", "Guts", "Intelligence"]) {
      if (totalFacilityBonus[cardType]["Total support cards"] > 0) {
        totalFacilityBonus[cardType]["Turns To Max Bond"] /= totalFacilityBonus[cardType]["Total support cards"];
      }
    }
    ...
    for (const card of this.deck) {
      for (const [stat, value] of Object.entries(totalStatsGained)) {
        ...
        (totalStatsGained as any)[stat] += eventValue * (1 + eventEffectiveness);
      }
      totalEnergySpent += ((card.eventsStatReward as any).Energy || 0) * (1 + eventRecovery);
      totalStatsGained["Skill Points"]! += ((card.eventsStatReward as any).Potential || 0) * (1 + eventEffectiveness);
    ...
    for (const [name, stats] of Object.entries(baseTrainingStats)) {
      ...
      if (index === 4) { // Intelligence/Wit facility
        const witRecovery = totalFacilityBonus[name]["Wit Friendship Recovery"] !== -1 ? 
          (totalFacilityBonus[name]["Wit Friendship Recovery"] || 0) : 0;
        totalEnergySpent += energyCost * turnsToTrainAtThisFacility * (1 + witRecovery);
      } else {
    ...
    index = 0;
    for (const [name, stats] of Object.entries(baseTrainingStats)) {
      const turnsToTrainAtThisFacility = adjustedMaxTrainingTurns * trainingDistribution[index];
      ...
      const moodEffect = (totalFacilityBonus[name]["Mood Effect"] || 0) / 100 + 1;
      const trainingEffectivenessRaw = (totalFacilityBonus[name]["Training Effectiveness"] || 0);
      const trainingEffectiveness = Math.max(trainingEffectivenessRaw / 100 + 1, 1); // Ensure it's never below 1
      const friendshipBonus = (totalFacilityBonus[name]["Friendship Bonus"] || 0) / 100 + 1;

      // Debug logging
      if (isNaN(trainingEffectivenessRaw)) {
        console.warn(`Training effectiveness is NaN for ${name}:`, {
          raw: trainingEffectivenessRaw,
          facilityBonus: totalFacilityBonus[name]["Training Effectiveness"],
      ...
      for (let turn = 0; turn < Math.ceil(turnsToTrainAtThisFacility); turn++) {
        ...
        const rainbow = turn >= (totalFacilityBonus[name]["Turns To Max Bond"] || 0) && 
                       (totalFacilityBonus[name]["Total support cards"] || 0) > 0;

        let averageStatsPerTurn: number[];
        if (rainbow) {
          averageStatsPerTurn = coreStats.map(stat => 
            Math.floor(stat * facilityMultiplier * moodBonus * moodEffect * trainingEffectiveness * friendshipBonus)
          );
        } else {
          averageStatsPerTurn = coreStats.map(stat => 
            Math.floor(stat * facilityMultiplier * moodBonus * trainingEffectiveness)
          );
        }
        ...
        if (turn === Math.ceil(turnsToTrainAtThisFacility) - 1) {
          ...
          if (fraction > 0) {
            averageStatsPerTurn = averageStatsPerTurn.map(x => x * fraction);
          }
        }

        // Add facility bonuses
        averageStatsPerTurn[0] += totalFacilityBonus[name]["Speed Bonus"] || 0;
        averageStatsPerTurn[1] += totalFacilityBonus[name]["Stamina Bonus"] || 0;
        averageStatsPerTurn[2] += totalFacilityBonus[name]["Power Bonus"] || 0;
        averageStatsPerTurn[3] += totalFacilityBonus[name]["Guts Bonus"] || 0;
        averageStatsPerTurn[4] += totalFacilityBonus[name]["Wit Bonus"] || 0;
    ...
    totalStatsGained["Skill Points"]! += finaleRace[5] * 3 + careerRace[5] * 8;
