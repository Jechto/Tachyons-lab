...
export class SupportCard {
  ...
  constructor(id: number, limitBreak: number, allData: CardData[]) {
    ...
    this.eventsStatReward = this.findBestEventChoice(cardData.all_events || { chain_events: [], dates: [] });

    // Initialize card bonus with all effect names set to -1
    const effectNames = [
      "Friendship Bonus", "Mood Effect", "Speed Bonus", "Stamina Bonus", "Power Bonus",
      "Guts Bonus", "Wit Bonus", "Training Effectiveness", "Initial Speed", "Initial Stamina",
      "Initial Power", "Initial Guts", "Initial Wit", "Initial Friendship Gauge", "Race Bonus",
      "Fan Bonus", "Hint Levels", "Hint Frequency", "Specialty Priority", "Max Speed",
      "Max Stamina", "Max Power", "Max Guts", "Max Wit", "Event Recovery", "Event Effectiveness",
      "Failure Protection", "Energy Cost Reduction", "Minigame Effectiveness", "Skill Point Bonus",
    ...
    for (const uniqueEffect of uniqueEffects) {
      const lbUnlockLevel = SupportCard.lmbToName[uniqueEffect.level_unlocked] ?? 0;
        ...
        for (const choice of arrowEvent.choices || []) {
          ...
          for (const reward of choice.rewards || []) {
            ...
            if (rtype in currentStatsForChoice) {
              currentStatsForChoice[rtype] += parseSignedInt(reward.value);
            }
          }

          const evalResult = this.evalStatArray(currentStatsForChoice);
  ...
  private parseCondition(conditionStr?: string): any {
    ...
    const parseAtom = (atom: string) => {
      ...
      if (!match) {
        throw new Error(`Invalid condition: ${atom}, full string: ${conditionStr}`);
      }
      const op = match[1];
  ...
  public extractSkillHints(): HintEvaluation[] {
    ...
    for (const cardHint of this.hints) {
      ...
      allHints.push({
        ...
        running_style_trigger: this.parseTrigger(condition, "running_style"),
        distance_type_trigger: this.parseTrigger(condition, "distance_type")
      });
    }
  ...
  public evaluateCardHints(
    ...
    const hintFreq = 0.075 * (this.cardBonus["Hint Frequency"] + 100) / 100;
    ...
    const usefulHintsRate = cardHints.length > 0 ? usefulHintCount / cardHints.length : 0;
    const hintsFromTraining = maxTrainingTurns * hintFreq * hintLevels * usefulHintsRate;
